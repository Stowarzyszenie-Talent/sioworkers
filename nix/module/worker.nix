...

{
  options.services.sioworker = {
    ...
    filetrackerUrl = lib.mkOption {
      default =
        if ((builtins.hasAttr "filetracker" config.services) && config.services.filetracker.enable) then
    ...
    filetrackerCache = lib.mkOption {
      default = "/var/cache/sioworker/filetracker";
  ...
  config =
    let
      ...
      autoFiletrackerCache = lib.hasPrefix "/var/cache/sioworker" cfg.filetrackerCache;
    in
    lib.mkIf cfg.enable {
      services.filetracker-cache-cleaner.paths = lib.mkIf autoFiletrackerCache [ cfg.filetrackerCache ];
      ...
      systemd.services.sioworker = {
        ...
        script = ''
          ${python.pkgs.twisted}/bin/twistd --nodaemon --pidfile=/run/sioworker/sioworker.pid worker \
              --can-run-cpu-exec \
